...
namespace TeenyPath {
    ...
    path::resolve_absolute() const {
...
#elif defined(_WIN32)
        HANDLE hFile = CreateFileW(this->wstring().c_str(),   // file to open
        ...
        {
            return path(""); //throw std::runtime_error("Could not get handle: " + m_path);
        }
        ...
        {
            CloseHandle(hFile);
            return path(""); //throw std::runtime_error("Failed to resolve path for: " + m_path);
        }

        std::unique_ptr<wchar_t[]> buffer = std::make_unique<wchar_t[]>(pathLength);
        DWORD result = GetFinalPathNameByHandleW(hFile,             // handle to file
        ...
        {
            return path(""); //throw std::runtime_error("Failed to resolve path for: " + this->string());
        }
    ...
    ls(const path& given) {
        ...
        HANDLE findHandle = FindFirstFileW(preppedFilePath, &findFileData);
